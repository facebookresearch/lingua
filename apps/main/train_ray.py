...
from ray_utils.init.ray_init import RayInitConfig, ray_init
...
def validate_train_args(args: TrainArgs, output_size: int):
    ...
    if (
        args.distributed.dp_replicate * args.distributed.dp_shard * args.distributed.tp_size
        != get_world_size()
    ):
        ...
        if args.distributed.fsdp_type == "no_shard":
            assert (
                args.distributed.dp_shard == 1 and args.distributed.dp_replicate == get_world_size()
            )
...
def main():
    ...
    for i in range(cfg.distributed.num_nodes):
        ...
        pg = scaling_config.as_placement_group_factory().to_placement_group()
        worker_cls = RayTrain.options(
